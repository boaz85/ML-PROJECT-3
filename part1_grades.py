...
# 1.
def load_data(path):
    return pd.read_csv(path).dropna(subset=['finalgrade'])
...
class TrainData(object):
    ...
    def __init__(self, data):
        self._raw_data = data
        self._df = data.drop('finalgrade', axis=1)
        self._labels = data['finalgrade']
    ...
    def _convert_categorical(self, column):
        mean_by_col = self._raw_data.groupby(column)['finalgrade'].mean().sort_values()
